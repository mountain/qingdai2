...
class WindDrivenSlabOcean:
    def __init__(
        ...
        self.r_bot = float(
            os.getenv("QD_R_BOT", "2.0e-5")
        )  # bottom drag (s^-1) stronger default for slab ocean
        self.rho_a = float(os.getenv("QD_RHO_A", "1.2"))  # air density for wind stress
        self.vcap = float(
            os.getenv("QD_WIND_STRESS_VCAP", "15.0")
        )  # cap for wind speed in stress (m/s)
        self.tau_scale = float(
            os.getenv("QD_TAU_SCALE", "0.2")
        )  # momentum transfer efficiency to slab
        ...
        self.max_u_cap = float(
            os.getenv("QD_OCEAN_MAX_U", "3.0")
        )  # sanity cap (ocean currents, vector cap)
        ...
        self.coslat = np.maximum(
            np.cos(self.lat_rad), 0.5
        )  # stronger cap to avoid polar metric blow-up
    ...
    def _hyperdiffuse(self, F: np.ndarray, dt: float, k4, n_substeps: int = 1) -> np.ndarray:
        ...
        try:
            if _jax_enabled():
                return _to_np(
                    _j_hyp(F, k4, dt, n_substeps, self.dlat, self.dlon, self.coslat, self.a)
                )
        except Exception:
            pass
    ...
    def _polar_vector_average_fill(
        self, u: np.ndarray, v: np.ndarray, ocean_mask: np.ndarray
    ) -> None:
        """
    ...
    def step(
        ...
        for _sub in range(n_sub):
            ...
            deta_dlam = (np.roll(self.eta, -1, axis=1) - np.roll(self.eta, 1, axis=1)) / (
                2.0 * self.dlon
            )
            deta_dphi = (np.roll(self.eta, -1, axis=0) - np.roll(self.eta, 1, axis=0)) / (
                2.0 * self.dlat
            )
            ...
            try:
                ...
                s = np.clip(
                    (lat_deg - self.polar_lat0) / max(1e-6, 90.0 - self.polar_lat0), 0.0, 1.0
                )
            ...
            if (self.diff_every > 0) and (self._step % self.diff_every == 0):
                ...
                k4_u = (
                    float(os.getenv("QD_OCEAN_K4_U")) if ("QD_OCEAN_K4_U" in os.environ) else k4_map
                )
                k4_v = (
                    float(os.getenv("QD_OCEAN_K4_V")) if ("QD_OCEAN_K4_V" in os.environ) else k4_map
                )
                k4_eta = (
                    float(os.getenv("QD_OCEAN_K4_ETA"))
                    if ("QD_OCEAN_K4_ETA" in os.environ)
                    else (0.5 * k4_map)
                )
            ...
            if (
                (self.shapiro_n > 0)
                and (self.shapiro_every > 0)
                and (self._step % self.shapiro_every == 0)
            ):
            ...
            if use_qnet and (Q_net is not None):
                ...
                if ice_mask is not None:
                    ...
                    if ice_qfac > 0.0:
                        Ts_new = np.where(
                            ice_mask_local, Ts_new + sub_dt * ice_qfac * heat_tendency, Ts_new
                        )
                    self.Ts = Ts_new
                else:
            ...
            if self.outlier_method == "mean4":
                ...
                u_n = np.roll(self.uo, -1, axis=0)
                u_s = np.roll(self.uo, 1, axis=0)
                u_e = np.roll(self.uo, -1, axis=1)
                u_w = np.roll(self.uo, 1, axis=1)
                v_n = np.roll(self.vo, -1, axis=0)
                v_s = np.roll(self.vo, 1, axis=0)
                v_e = np.roll(self.vo, -1, axis=1)
                v_w = np.roll(self.vo, 1, axis=1)
                ...
                if (self._step % every) == 0:
                    ...
                    if Q_net is not None:
                        if ice_mask is not None:
                            ...
                            if ice_qfac > 0.0:
                                eff_Q += np.where(
                                    (self.land_mask == 0) & (ice_mask), ice_qfac * Q_net, 0.0
                                )
                        else:
                    ...
                    lat_deg = np.abs(np.rad2deg(self.lat_rad))
                    polar_mask = (lat_deg >= float(os.getenv("QD_OCEAN_POLAR_LAT", "60.0"))) & (
                        self.land_mask == 0
                    )
                    if np.any(polar_mask):
                        ...
                        if Q_net is not None:
                            if ice_mask is not None:
                                ...
                                if ice_qfac > 0.0:
                                    eff_Qp += np.where(
                                        polar_mask & (ice_mask), ice_qfac * Q_net, 0.0
                                    )
                            else:
                            ...
                            Qp_mean = 0.0
                        dTp = (self.Ts - getattr(self, "_Ts_prev_for_diag_p", self.Ts)) / max(
                            1e-12, dt
                        )
                    ...
                    print(
                        f"[OceanE] ⟨Q_net⟩={Q_mean:+.2f} W/m^2 | implied={implied:+.2f} | resid={resid:+.2f}  "
