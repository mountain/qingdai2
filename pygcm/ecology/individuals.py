...
class IndividualPool:
    ...
    def __init__(
        ...
        *,
        ...
        self.cfg = IndividualPoolConfig(
            ...
            substeps_per_day=max(
                1, int(os.getenv("QD_ECO_INDIV_SUBSTEPS_PER_DAY", str(substeps_per_day)))
            ),
            nb_max=(
                int(os.getenv("QD_ECO_SPECTRAL_BANDS", "16"))
                if os.getenv("QD_ECO_SPECTRAL_BANDS")
                else 16
            ),
        ...
        self.sp_weights = (
            (sp_weights / ssum)
            if ssum > 0
            else np.full((self.ns,), 1.0 / float(self.ns), dtype=float)
        )
        ...
        self.indiv_species_id = rng.choice(
            np.arange(self.ns, dtype=np.int32), size=self.n_indiv, p=self.sp_weights
        )
        ...
        if species_R.shape[1] != self.nb:
            ...
            if species_R.shape[1] > self.nb:
                ...
                species_R = np.pad(
                    species_R, ((0, 0), (0, self.nb - species_R.shape[1])), mode="edge"
                )
        # Small per-individual jitter to encourage micro-diversity
        jitter = 0.02
        Ab = species_R[self.indiv_species_id, :] + rng.normal(
            0.0, jitter, size=(self.n_indiv, self.nb)
        )
        ...
        if self.cfg.diag:
            print(
                f"[EcoIndiv] initialized: sample_frac={self.cfg.sample_frac:.3f}, "
    ...
    def try_substep(
        ...
        self.indiv_water_stress_days[mask_stress] += float(self._substep_period) / float(
            day_length_seconds
        )
    ...
    def step_daily(
        ...
        if stress_penalty > 0.0:
            ...
            mean_stress = np.divide(
                stress_s_c, cnt_s_c, out=np.zeros_like(stress_s_c), where=(cnt_s_c > 0)
            )
        ...
        lai_grow = float(
            os.getenv("QD_ECO_LAI_GROWTH_RATE", "0.002")
        )  # growth when energy above median
        lai_decay = float(os.getenv("QD_ECO_LAI_DECAY_RATE", "0.001"))  # decay with stress
        recruit_frac = float(
            os.getenv("QD_ECO_LAI_RECRUIT_FRAC", "0.2")
        )  # fraction of positive dLAI that spills to neighbors
        ...
        for ci in range(C):
            ...
            try:
                mean_stress_cell = (
                    float(np.sum(mean_stress[:, ci] * wk)) if "mean_stress" in locals() else 0.0
                )
            except Exception:
            ...
            scale = (
                (new_total / (total_old + 1e-12))
                if total_old > 0.0
                else (new_total / max(lai_max_cell, 1.0))
            )
        ...
        try:
            if int(os.getenv("QD_ECO_INDIV_SEED_COUPLE", "1")) == 1 and hasattr(pop, "seed_bank"):
                ...
                E_tot_cells = denom  # [C]
                repro_frac = float(
                    getattr(pop, "repro_fraction", float(os.getenv("QD_ECO_REPRO_FRACTION", "0.2")))
                )
                seed_energy = float(
                    getattr(pop, "seed_energy", float(os.getenv("QD_ECO_SEED_ENERGY", "1.0")))
                )
                ...
                seeds_cells = (
                    np.maximum(0.0, repro_frac)
                    * np.maximum(0.0, E_tot_cells)
                    / max(seed_energy, 1e-12)
                )
                ...
                for ci in range(C):
                    j = int(self.sample_j[ci])
                    i = int(self.sample_i[ci])
                    pop.seed_bank[j, i] = np.clip(
                        pop.seed_bank[j, i] + seeds_cells[ci], 0.0, bank_max
                    )
        except Exception:
            pass
        ...
        self.indiv_water_stress_days[~ok] = np.minimum(
            self.indiv_water_stress_days[~ok] + 1.0, 365.0
        )

        if self.cfg.diag:
            beta_hint = float(np.mean(np.max(W_s_c, axis=0)))
            print(
                f"[EcoIndiv] daily applied to {self.n_cells} cells × {self.per_cell} indiv; "
